...
if (isset($_GET['galleryid'])) {
	// get gallery information: title and description
  ...
  if(!empty($wpdb->get_results($query))) {
     foreach( $wpdb->get_results($query) as $key => $gallery_record) {
   $gallery_title_old = $gallery_record -> title;
   $gallery_description_old =$gallery_record -> description;
...
<div class="form_upload">
  ...
  if (isset($_GET['galleryid'])) {
    echo "<input  type='hidden' name='galleryid' id='galleryid' value = '".$_GET['galleryid']."'>";
    if ($_POST['gallery_name']) {$title_gall= stripslashes($_POST['gallery_name']); } else {$title_gall= $gallery_title_old;} 
    if ($_POST['gallery_description']) {$desc_gall= stripslashes($_POST['gallery_description']);} else {$desc_gall= $gallery_description_old;}  
  }?>

	 <p><input type='text' name = 'gallery_name' id = 'gallery_name' value = "<?php echo $title_gall?>" placeholder='Title:'/></p>
	 <p><input type='text' name = 'gallery_description' id = 'gallery_description' value = "<?php echo $desc_gall ?> " placeholder='Description:'/></p>
  <?php 
    // scheme id list
